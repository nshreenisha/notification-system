  ...
  try {
    ...
    return rows.map(row => {
      try {
        ...
        const messageData = typeof row.message_data === 'string' 
        ...
        console.warn(`⚠️ Failed to parse message data for message ${row.id}:`, parseError.message)
        // Return message with raw data if parsing fails
        return {
          id: row.id,
          type: row.type,
