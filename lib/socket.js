  ...
  if (!io) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('join-user-room', async (userId) => {
        ...
        try {
          const mysqlDB = await import('./mysql-database.js')
          const offlineMessages = await mysqlDB.getOfflineMessages(userId)

          if (offlineMessages.length > 0) {
            console.log(`📦 Found ${offlineMessages.length} offline messages for user ${userId}`)
            ...
            for (const msg of offlineMessages) {
              ...
              if (msg.type === 'content-update') {
                socket.emit('content-update', msg.data)
              } else {
                socket.emit('notification', msg.data)
              }

              processedIds.push(msg.id)
            }

            // Clear processed messages from DB
            if (processedIds.length > 0) {
              await mysqlDB.clearOfflineMessages(userId, processedIds)
            }
          }
        } catch (error) {
      ...
      // NEW: User joins role-based room (e.g., chef, manager)
      socket.on('join-role-room', (data) => {
        const { companyId, role } = data

        if (!companyId || !role) {
          socket.emit('join-error', { error: 'Company ID and Role are required' })
          return
        }

        // Construct room name: company-{id}-{role}
        const roleRoom = `company-${companyId}-${role}`
        socket.join(roleRoom)

        // Also register as a "role" user for this company (optional, helps with tracking)
        // socket.role = role
        // socket.companyId = companyId

        socket.emit('joined-role-room', {
          success: true,
          companyId,
          role,
          room: roleRoom,
...
// Helper function to send notification to specific user (with duplicate prevention and offline support)
async function sendToUser(userId, message, type = 'info', title = null) {
  if (!io) {
    throw new Error('Socket.io not initialized')
  }
  ...
  if (roomSockets && roomSockets.size > 0) {
    ...
    // User not connected - Store offline message
    console.log(`❌ User ${userId} not connected for direct notification - Storing offline`)

    // Import dynamically to avoid circular dependencies if any
    try {
      const mysqlDB = await import('./mysql-database.js')
      const messageToStore = { ...notification, offline: true }
      await mysqlDB.storeOfflineMessage(userId, messageToStore, type)
    } catch (dbError) {
      console.error('❌ Failed to store offline message:', dbError)
    }
  }
